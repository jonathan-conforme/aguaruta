...
class TripService
{
        ...
        return DB::transaction(function () use ($data) {
            ...
            if (!empty($data['products'])) {
               ...
               foreach ($data['products'] as $item) {
                ...
                'products' => "Stock insuficiente para {$product->name}. Disponible: {$product->current_stock} unidades"
                ]);
                }
                ...
                InventoryMovement::create([
                    ...
                    'quantity'    => $realUnits,
                    'description' =>
                    "Carga de camión para Despacho #{$trip->id} " .
                ...
                $trip->products()->attach($productsToAttach);
            }
      ...
      public function updateTrip(Trip $trip, array $data)
    {
        return DB::transaction(function () use ($trip, $data) {
            ...
            // Aseguramos que 'products' no se intente guardar en la tabla principal de trips
            $tripData = collect($data)->except('products')->toArray();
            $trip->update($tripData);

            // 3. OBTENER ESTADO ACTUAL Y NUEVO
            $oldProducts = $trip->products->keyBy('id'); // Colección indexada por product_id
            $newProductsData = collect($data['products'] ?? [])->keyBy('product_id');
            
            // Obtenemos todos los IDs únicos (los que ya estaban + los nuevos)
            $allProductIds = $oldProducts->keys()->merge($newProductsData->keys())->unique();

            $syncData = [];

            // 4. CALCULAR DIFERENCIAS Y MOVER INVENTARIO SOLO SI ES NECESARIO
            foreach ($allProductIds as $productId) {
                $oldPackages = $oldProducts->has($productId) ? $oldProducts[$productId]->pivot->initial_quantity : 0;
                $newPackages = $newProductsData->has($productId) ? $newProductsData[$productId]['quantity'] : 0;
