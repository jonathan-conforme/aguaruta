...
class TripService
{
    ...
    public function createTrip(array $data)
    {
        ...
        return DB::transaction(function () use ($data) {
            ...
            if (! empty($data['products'])) {
                ...
                foreach ($data['products'] as $item) {
                    ...
                    InventoryMovement::create([
                        ...
                        'type' => 'out',
                        'quantity' => $realUnits,
                        'description' => "Carga de camión para Despacho #{$trip->trip_number} ".
            ...
            // 5. Notificar al Repartidor (Driver) si fue asignado
            if ($trip->driver) {
                $trip->driver->notify(new ViajeAsignadoNotification($trip));
            }
    ...
    public function startTrip(int $tripId, float $initialCash, int $shiftId)
    {
        return DB::transaction(function () use ($tripId, $initialCash, $shiftId) {
            ...
            $admins = User::where('company_id', $trip->company_id)
                ->where('role', 'admin')
                ->get();

            foreach ($admins as $admin) {
                $admin->notify(new ViajeIniciadoNotification($trip));
            }
